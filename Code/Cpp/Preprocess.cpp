...
void preprocess(cv::Mat &imgOriginal, cv::Mat &imgGrayscale, cv::Mat &imgThresh, uint_xy_t PreprocessGaussKernel, unsigned int PreprocessThreshBlockSize,
    ...
    if (OpMode == "police") {
        imgGrayscale = vectorOfHSVImages[1];
    } else {
    ...
    cv::Size gaussKernel;
    if (phase == "platesDet") {
        gaussKernel = cv::Size(PreprocessGaussKernel.get_x()+2, PreprocessGaussKernel.get_y()+2);
    } else {
        gaussKernel = cv::Size(PreprocessGaussKernel.get_x(), PreprocessGaussKernel.get_y());
    }
...
cv::Mat maximizeContrast(cv::Mat &imgGrayscale, uint_xy_t PreprocessMorphKernel) {
    ...
    cv::Mat structuringElement = cv::getStructuringElement(CV_SHAPE_RECT, cv::Size(PreprocessMorphKernel.get_x(), PreprocessMorphKernel.get_y()));
...
cv::Mat imageEnhancement(cv::Mat &imgOriginal, double clahe_clipLimit, uint_xy_t clahe_tileGridSize, double gamma, imgEnhancement_mode_t mode, bool debugMode) {
    ...
    cv::Mat imgOut, imgTmp;
    
    switch (mode) {
        case IMG_ENHANCEMENT_MODE_WARM_CLAHE_SAT_GAMMA:
            imgOut = imageEnhancement_mode0(imgOriginal, clahe_clipLimit, clahe_tileGridSize, gamma);
            break;
        case IMG_ENHANCEMENT_MODE_HIST_EQUALIZATION:
            imgOut = imageEnhancement_mode1(imgOriginal);
            break;
        case IMG_ENHANCEMENT_MODE_MIXED0:
            imgTmp = imageEnhancement_mode0(imgOriginal, clahe_clipLimit, clahe_tileGridSize, gamma);
            imgOut = imageEnhancement_mode1(imgTmp); 
            break;
        case IMG_ENHANCEMENT_MODE_MIXED1:
            imgTmp = imageEnhancement_mode1(imgOriginal);
            imgOut = imageEnhancement_mode0(imgTmp, clahe_clipLimit, clahe_tileGridSize, gamma);
            break;
        default:
            error("Unsupported ImageEnhancement mode");
    }
...
cv::Mat imageEnhancement_mode0(cv::Mat &imgOriginal, double clahe_clipLimit, uint_xy_t clahe_tileGridSize, double gamma) {
    ...
    clahe->setTilesGridSize(cv::Size(clahe_tileGridSize.get_x(),clahe_tileGridSize.get_y()));
    clahe->apply(c_b, B);
    clahe->apply(g, G);
    clahe->apply(c_r, R);
    ...
    cv::Mat imgHSV, c_s, imgSat, imgOut;
    std::vector<cv::Mat> vectorOfHSVImages;
    ...
    cv::LUT(imgSat, gamma3_lut, imgOut);
...
cv::Mat imageEnhancement_mode1(cv::Mat &imgOriginal) {
    ...
    cv::Mat imgOut;
    cvtColor(imgOriginal, imgOut, cv::COLOR_BGR2YCrCb);

    // Split the image into 3 channels; Y, Cr and Cb channels respectively and store it in a std::vector
    std::vector<cv::Mat> vec_channels;
    cv::split(imgOut, vec_channels); 

    // Equalize the histogram of only the Y channel 
    cv::equalizeHist(vec_channels[0], vec_channels[0]);

    // Merge 3 channels in the vector to form the color image in YCrCB color space.
    cv::merge(vec_channels, imgOut); 
        
    //Convert the histogram equalized image from YCrCb to BGR color space again
    cv::cvtColor(imgOut, imgOut, cv::COLOR_YCrCb2BGR);
...
cv::Mat perspectiveWarp(cv::Mat &imgOriginal, uint_xy_t pnt0, uint_xy_t pnt1, uint_xy_t pnt2, uint_xy_t pnt3) {
    ...
    // 4 points that defines the mapping, from top-left in clockwise order):
    cv::Point2f inputQuad[4] = {
      cv::Point2f( 0, 0 ),
      cv::Point2f( imgOriginal.cols-1, 0 ),
      cv::Point2f( imgOriginal.cols-1,imgOriginal.rows-1 ),
      cv::Point2f( 0,imgOriginal.rows-1 )
    };

    cv::Point2f outputQuad[4] {
      cv::Point2f( pnt0.get_x(), pnt0.get_y() ),
      cv::Point2f( pnt1.get_x(), pnt1.get_y() ),
      cv::Point2f( pnt2.get_x(), pnt2.get_y() ),
      cv::Point2f( pnt3.get_x(), pnt3.get_y() ),
    };
    ...
    cv::warpPerspective(imgOriginal, imgOut, lambda, imgOriginal.size());
