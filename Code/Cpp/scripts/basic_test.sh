...
LPR_test() {
  if [ "$# == 3" ]; then
    res=`build/lpr -i $1 $3 --batch | grep -w $2`
  else
    res=`build/lpr -i $1 --batch | grep -w $2`
  fi
  if [ "$res" ]; then
    echo "$1 PASSED!"
  else
...
LPR_test "$base/LPR/Database/Israel/15.jpg"  "9599974" "--MinHistNormThr=0"           "White background"
...
LPR_test "$base/LPR/Database/Israel/23.jpg"  "23383201" "--blueMaxThrH=50"
